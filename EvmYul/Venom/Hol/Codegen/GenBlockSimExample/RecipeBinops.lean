...
/-! ### ✅ A BLOBHASH (blob-hash read) via the recipe route — `codegen_correct_bhhFn_recipeW`

The block-environment read `BLOBHASH` (the newly-wired 13th `RegularStepG` arm, `asmStep_blobhash_ok` /
`bodyStep_blobhash`), twin of `BLOBHASH`/`TLOAD`:

```
entry:  %a = CALLVALUE ; %b = CALLVALUE ; %c = BLOBHASH %a ; JMP next   -- a,b,c all live → delivered
next:   %e = MUL %b %c ; RETURN %e, %a                               -- MUL consumes the TOP two (b,c)
```

`BLOBHASH %a` is a 1-input read (`RegularStepG`'s BLOBHASH arm, emitted `DUP2 ; BLOBHASH`), leaving `[a, b, c]`
with `c = bhhVal s` (the blob hash at index `callvalue` — context-dependent, generally NON-zero, so
`c` cannot itself be a RETURN operand). In `next` the consuming `MUL %b %c` eats the top two (`b, c`);
since `b = 0`, `e = 0 * c = 0` (via `uint256_zero_mul`, regardless of `c`), emptying the return window.
`a` (bottom) is never buried — no f1a reorder. `entry` via `hsupplyW_regularJmp`, `next` via
`hsupplyW_regularReturnTo` fed the consuming `MUL` (`bhhNext_ready`). -/

def bhhCvA : Instruction := { id := 0, opcode := Opcode.CALLVALUE, operands := [], outputs := ["a"] }
def bhhCvB : Instruction := { id := 1, opcode := Opcode.CALLVALUE, operands := [], outputs := ["b"] }
def bhhLoad : Instruction :=
  { id := 2, opcode := Opcode.BLOBHASH, operands := [Operand.Var "a"], outputs := ["c"] }
def bhhJmp : Instruction := { id := 3, opcode := Opcode.JMP, operands := [Operand.Label "next"], outputs := [] }
def bhhMul : Instruction :=
  { id := 4, opcode := Opcode.MUL, operands := [Operand.Var "b", Operand.Var "c"], outputs := ["e"] }
def bhhRet : Instruction :=
  { id := 5, opcode := Opcode.RETURN, operands := [Operand.Var "e", Operand.Var "a"], outputs := [] }
def bhhEntry : BasicBlock := { label := "entry", instructions := [bhhCvA, bhhCvB, bhhLoad, bhhJmp] }
def bhhNext : BasicBlock := { label := "next", instructions := [bhhMul, bhhRet] }
def bhhFn : IrFunction := { name := "main", blocks := [bhhEntry, bhhNext] }
def bhhCtx : VenomContext := { functions := [bhhFn], entry := some "main" }

theorem bhh_unresolved_asm : executePlan (generateFnPlan bhhFn 0 0).get!.1 =
    [AsmInst.AsmLabel "entry", AsmInst.AsmOp "CALLVALUE", AsmInst.AsmOp "CALLVALUE",
     AsmInst.AsmOp "DUP2", AsmInst.AsmOp "BLOBHASH", AsmInst.AsmPushLabel "next",
     AsmInst.AsmOp "JUMP", AsmInst.AsmLabel "next", AsmInst.AsmOp "MUL", AsmInst.AsmOp "RETURN"] := by rfl

/-- The blob hash BLOBHASH reads at index `a` (= the call value); context-dependent, generally NON-zero. -/
abbrev bhhVal (s : VenomState) : bytes32 := blobhashOf s.txCtx s.callCtx.callvalue.toNat

/-- entry's body `[CV a; CV b; BLOBHASH %a]` as a `RegularBodyH`: two `cv_step`s then the
    BLOBHASH read via `RegularStepG`'s BLOBHASH arm (`asmStep_tload_ok`), output live. -/
theorem bhhEntry_body {lo : AssocList String Nat} {offsetToPc : AssocList Nat Nat} :
    RegularBodyH lo ["a", "b", "c"] offsetToPc
      (asmResolve (executePlan (generateFnPlan bhhFn 0 0).get!.1)).1 1 [bhhCvA, bhhCvB, bhhLoad] [] := by
  refine ⟨cv_step (out := "a") (S := []) rfl (by decide) (by decide),
          cv_step (out := "b") (S := ["a"]) rfl (by decide) (by decide), ?_, trivial⟩
  refine Or.inl ⟨?_, by decide⟩
  exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr
    ⟨"a", "c", rfl, by decide, rfl, fun _ => rfl, rfl, rfl,
     by decide, by decide, by decide, by decide, fun _ h hg => asmStep_blobhash_ok h hg⟩)))))))))))

/-- Body-end state of `bhhEntry`: `a,b := callvalue`, `c := bhhVal s` (the block-hash read at index `a`). -/
abbrev bhhEntrySEnd (s : VenomState) : VenomState :=
  { updateVar "c" (bhhVal s)
      { updateVar "b" s.callCtx.callvalue
          { updateVar "a" s.callCtx.callvalue { s with instIdx := 0 } with instIdx := 1 }
        with instIdx := 2 }
    with instIdx := 3 }

theorem bhhEntry_thread (s : VenomState) :
    execBodyThread [bhhCvA, bhhCvB, bhhLoad] 0 { s with instIdx := 0 } = some (bhhEntrySEnd s) := by
  have hla : lookupVar "a" ({ updateVar "b" s.callCtx.callvalue
        { updateVar "a" s.callCtx.callvalue { s with instIdx := 0 } with instIdx := 1 }
      with instIdx := 2 } : VenomState) = some s.callCtx.callvalue := by
    show lookupVar "a" (updateVar "b" s.callCtx.callvalue
  ...
  have hlb : lookupVar "b" ({ updateVar "b" s.callCtx.callvalue
      ...
      with instIdx := 2 } : VenomState) = some s.callCtx.callvalue :=
    lookupVar_updateVar_self _ _ _
  have hsub : stepInstBase bhhLoad ({ updateVar "b" s.callCtx.callvalue
        { updateVar "a" s.callCtx.callvalue { s with instIdx := 0 } with instIdx := 1 }
      with instIdx := 2 } : VenomState) = ExecResult.OK (updateVar "c" (bhhVal s)
        ({ updateVar "b" s.callCtx.callvalue
          { updateVar "a" s.callCtx.callvalue { s with instIdx := 0 } with instIdx := 1 }
        with instIdx := 2 })) := by
    have he : evalOperand (Operand.Var "a") ({ updateVar "b" s.callCtx.callvalue
        { updateVar "a" s.callCtx.callvalue { s with instIdx := 0 } with instIdx := 1 }
      with instIdx := 2 } : VenomState) = some s.callCtx.callvalue := hla
    simp only [bhhLoad, stepInstBase, execRead1, he]
    rfl
  have h1 : stepInstBase bhhCvA { s with instIdx := 0 }
      = ExecResult.OK (updateVar "a" s.callCtx.callvalue { s with instIdx := 0 }) := rfl
  have h2 : stepInstBase bhhCvB ({ updateVar "a" s.callCtx.callvalue { s with instIdx := 0 }
        with instIdx := 1 } : VenomState)
      = ExecResult.OK (updateVar "b" s.callCtx.callvalue
          ({ updateVar "a" s.callCtx.callvalue { s with instIdx := 0 } with instIdx := 1 })) := rfl
  show (match stepInstBase bhhCvA { s with instIdx := 0 } with
        | ExecResult.OK s' => execBodyThread [bhhCvB, bhhLoad] 1 { s' with instIdx := 1 }
        | _ => none) = some (bhhEntrySEnd s)
  rw [h1]
  show (match stepInstBase bhhCvB ({ updateVar "a" s.callCtx.callvalue { s with instIdx := 0 }
          with instIdx := 1 }) with
        | ExecResult.OK s' => execBodyThread [bhhLoad] 2 { s' with instIdx := 2 }
        | _ => none) = some (bhhEntrySEnd s)
  rw [h2]
  show (match stepInstBase bhhLoad ({ updateVar "b" s.callCtx.callvalue
            { updateVar "a" s.callCtx.callvalue { s with instIdx := 0 } with instIdx := 1 }
          with instIdx := 2 }) with
        | ExecResult.OK s' => execBodyThread [] 3 { s' with instIdx := 3 }
        | _ => none) = some (bhhEntrySEnd s)
...
/-- `bhhEntry`'s OK result is `jumpTo "next"` of the body-end state (fuel ≥ 4). -/
theorem bhhEntry_runBlock (s : VenomState) (j : Nat) (hnh : s.halted = false) :
    runBlock (3 + (j + 1)) bhhCtx bhhEntry s = ExecResult.OK (jumpTo "next" (bhhEntrySEnd s)) :=
  runBlock_body_jmp bhhCtx bhhEntry j [bhhCvA, bhhCvB, bhhLoad] bhhJmp bhhCvA [bhhCvB, bhhLoad, bhhJmp] s
    (bhhEntrySEnd s) "next" rfl rfl rfl rfl (by decide)
    (by intro i hi; simp only [List.mem_cons, List.not_mem_nil, or_false] at hi
        rcases hi with rfl | rfl | rfl <;> decide)
    (bhhEntry_thread s) (by simpa [updateVar] using hnh)

/-- `next`'s consuming `MUL %b %c` (mirror order, `bytes32_mul_comm`) as a `BodyStepsReadyHTo`:
    it eats the top two `[b, c]` off the entry stack `[a, b, c]`, leaving `[a, e]`. -/
theorem bhhNext_ready {lo : AssocList String Nat} :
    BodyStepsReadyHTo lo (asmResolve (executePlan (generateFnPlan bhhFn 0 0).get!.1)).2
      (asmResolve (executePlan (generateFnPlan bhhFn 0 0).get!.1)).1
      (fun z p => generateRegularInstPlan exLiveness DfgAnalysis.empty exCfg bhhFn z.1
        ["e", "a"] false true "next" p)
      (fun _ => 1) ([bhhMul].zipIdx 0) ["a", "b", "c"] ["a", "e"] :=
  bodyStepsReadyHTo_singleton
    (bodyStepHTo_commBinopDeadMirror (liveness := exLiveness) (dfg := DfgAnalysis.empty)
      (cfg := exCfg) (fn := bhhFn) (curBbLabel := "next") (base := ["a"]) (x := "b") (y := "c")
      (out := "e") (idx := 0)
      rfl (by decide) bytes32_mul_comm (fun _ => rfl) rfl rfl (by decide) (by decide) (by decide)
      (by decide) (by decide) (fun _ h hg => asmStep_mul_ok h hg))

abbrev bhhNextSEnd (s : VenomState) (wb wc : bytes32) : VenomState :=
  { updateVar "e" (wb * wc) { s with instIdx := 0 } with instIdx := 1 }

theorem bhhNextSEnd_lookup (s : VenomState) (wb wc : bytes32) :
    lookupVar "e" (bhhNextSEnd s wb wc) = some (wb * wc)
    ∧ lookupVar "a" (bhhNextSEnd s wb wc) = lookupVar "a" s := by
  refine ⟨lookupVar_updateVar_self _ _ _, ?_⟩
  show lookupVar "a" (updateVar "e" (wb * wc) { s with instIdx := 0 }) = _
  rw [lookupVar_updateVar_ne _ _ _ _ (by decide)]; rfl

theorem bhhNext_thread (s : VenomState) (wb wc : bytes32)
    (hb : lookupVar "b" s = some wb) (hc : lookupVar "c" s = some wc) :
    execBodyThread [bhhMul] 0 { s with instIdx := 0 } = some (bhhNextSEnd s wb wc) := by
  have hb' : lookupVar "b" { s with instIdx := 0 } = some wb := hb
  have hc' : lookupVar "c" { s with instIdx := 0 } = some wc := hc
  have hstep : stepInstBase bhhMul { s with instIdx := 0 }
      = ExecResult.OK (updateVar "e" (wb * wc) { s with instIdx := 0 }) :=
    stepInstBase_binopVar (inst := bhhMul) (f := fun a b => a * b) (x := "b") (y := "c")
      (out := "e") rfl rfl rfl hb' hc'
  show (match stepInstBase bhhMul { s with instIdx := 0 } with
        | ExecResult.OK s' => execBodyThread [] 1 { s' with instIdx := 1 } | _ => none) = _
  rw [hstep]; rfl

/-- The MUL errors when `b` is undefined (the shape `runBlock_body_head_error` consumes). -/
theorem bhhMul_error_b (s : VenomState) (hb : lookupVar "b" s = none) :
    stepInstBase bhhMul { s with instIdx := 0 } = ExecResult.Error "undefined operand" := by
  have hb' : lookupVar "b" { s with instIdx := 0 } = none := hb
  simp [bhhMul, stepInstBase, execPure2, evalOperand, hb']

/-- The MUL errors when `c` is undefined (given `b` defined). -/
theorem bhhMul_error_c (s : VenomState) (wb : bytes32)
    (hb : lookupVar "b" s = some wb) (hc : lookupVar "c" s = none) :
    stepInstBase bhhMul { s with instIdx := 0 } = ExecResult.Error "undefined operand" := by
  have hb' : lookupVar "b" { s with instIdx := 0 } = some wb := hb
  have hc' : lookupVar "c" { s with instIdx := 0 } = none := hc
  simp [bhhMul, stepInstBase, execPure2, evalOperand, hb', hc']

/-- `next` halts when `a, b, c` are all defined: ADD then RETURN reads `e = b+c` and `a`. -/
theorem bhhNext_halts (s : VenomState) (j : Nat) (wb wc wa : bytes32)
    (hb : lookupVar "b" s = some wb) (hc : lookupVar "c" s = some wc)
    (ha : lookupVar "a" s = some wa) :
    runBlock (1 + (j + 1)) bhhCtx bhhNext s = ExecResult.Halt
      (haltState (setReturndata (readMemory (wb * wc).toNat wa.toNat (bhhNextSEnd s wb wc))
        (bhhNextSEnd s wb wc))) := by
  obtain ⟨he, haa⟩ := bhhNextSEnd_lookup s wb wc
  rw [ha] at haa
  exact runBlock_body_return bhhCtx bhhNext j [bhhMul] bhhRet bhhMul [bhhRet] s (bhhNextSEnd s wb wc)
    (Operand.Var "e") (Operand.Var "a") (wb * wc) wa rfl rfl rfl he haa rfl (by decide)
    (by intro i hi; simp only [List.mem_singleton] at hi; subst hi; decide)
    (bhhNext_thread s wb wc hb hc)

/-- `next` errors when `a` is undefined (given `b, c` defined): ADD succeeds, RETURN faults on `a`. -/
theorem bhhNext_error_a (s : VenomState) (j : Nat) (wb wc : bytes32)
    (hb : lookupVar "b" s = some wb) (hc : lookupVar "c" s = some wc) (ha : lookupVar "a" s = none) :
    runBlock (1 + (j + 1)) bhhCtx bhhNext s = ExecResult.Error "return: undefined operand" := by
  obtain ⟨he, haa⟩ := bhhNextSEnd_lookup s wb wc
  rw [ha] at haa
  refine runBlock_body_term bhhCtx bhhNext j [bhhMul] bhhRet bhhMul [bhhRet] s (bhhNextSEnd s wb wc)
    (ExecResult.Error "return: undefined operand") rfl rfl (by decide)
    (by intro i hi; simp only [List.mem_singleton] at hi; subst hi; decide)
    (bhhNext_thread s wb wc hb hc) ?_
  intro inst hget
  simp [execBlock, getInstruction_bodyEnd (by rfl : bhhNext.instructions = [bhhMul] ++ [bhhRet])
    (bhhNext_thread s wb wc hb hc), bhhRet, stepInstBase, evalOperand, he, haa, isExternalCall]

/-- lookupVar of `bhhEntrySEnd`: `a, b` are the call value, `c = callvalue - callvalue`. -/
theorem bhhEntrySEnd_lookup (s : VenomState) :
    lookupVar "a" (bhhEntrySEnd s) = some s.callCtx.callvalue
    ∧ lookupVar "b" (bhhEntrySEnd s) = some s.callCtx.callvalue
    ∧ lookupVar "c" (bhhEntrySEnd s) = some (bhhVal s) := by
  refine ⟨?_, ?_, ?_⟩
  · show lookupVar "a" (updateVar "c" (bhhVal s)
      (updateVar "b" s.callCtx.callvalue (updateVar "a" s.callCtx.callvalue { s with instIdx := 0 }))) = _
    rw [lookupVar_updateVar_ne _ _ _ _ (by decide), lookupVar_updateVar_ne _ _ _ _ (by decide),
      lookupVar_updateVar_self]
  · show lookupVar "b" (updateVar "c" (bhhVal s)
      (updateVar "b" s.callCtx.callvalue (updateVar "a" s.callCtx.callvalue { s with instIdx := 0 }))) = _
    rw [lookupVar_updateVar_ne _ _ _ _ (by decide), lookupVar_updateVar_self]
  · show lookupVar "c" (updateVar "c" (bhhVal s)
      (updateVar "b" s.callCtx.callvalue (updateVar "a" s.callCtx.callvalue { s with instIdx := 0 }))) = _
    rw [lookupVar_updateVar_self]

/-- The invariant: unhalted, zero call value, and at `next` the delivered `a, b, c` are all zero. -/
def bhhInv (s : VenomState) : Prop :=
  s.halted = false ∧ s.callCtx.callvalue = EvmYul.UInt256.ofNat 0 ∧
  (s.currentBb = "next" → lookupVar "a" s = some (EvmYul.UInt256.ofNat 0)
                        ∧ lookupVar "b" s = some (EvmYul.UInt256.ofNat 0)
                        ∧ lookupVar "c" s = some (bhhVal s))

theorem bhhInv_pres : ∀ bb ∈ bhhFn.blocks, ∀ (s s' : VenomState) (f' : Nat),
    bhhInv s → runBlock f' bhhCtx bb s = ExecResult.OK s' → bhhInv s' := by
  intro bb hbb s s' f' hinv hrun
  obtain ⟨hnh, hcv, _⟩ := hinv
  simp only [bhhFn, List.mem_cons, List.not_mem_nil, or_false] at hbb
  rcases hbb with rfl | rfl
  · -- entry: fuel ≤ 3 is out-of-fuel (Error); fuel ≥ 4 lands on `jumpTo "next" (bhhEntrySEnd s)`
    have hnt : ∀ inst ∈ [bhhCvA, bhhCvB, bhhLoad], isTerminator inst.opcode = false := by
      intro i hi; simp only [List.mem_cons, List.not_mem_nil, or_false] at hi
      rcases hi with rfl | rfl | rfl <;> decide
    match f' with
    | 0 => obtain ⟨e, he⟩ := runBlock_oof bhhCtx bhhEntry 0 [bhhCvA, bhhCvB, bhhLoad] bhhJmp bhhCvA
             [bhhCvB, bhhLoad, bhhJmp] s (bhhEntrySEnd s) rfl rfl (by decide) hnt (bhhEntry_thread s) (by decide)
           rw [he] at hrun; exact absurd hrun (by simp)
    | 1 => obtain ⟨e, he⟩ := runBlock_oof bhhCtx bhhEntry 1 [bhhCvA, bhhCvB, bhhLoad] bhhJmp bhhCvA
             [bhhCvB, bhhLoad, bhhJmp] s (bhhEntrySEnd s) rfl rfl (by decide) hnt (bhhEntry_thread s) (by decide)
           rw [he] at hrun; exact absurd hrun (by simp)
    | 2 => obtain ⟨e, he⟩ := runBlock_oof bhhCtx bhhEntry 2 [bhhCvA, bhhCvB, bhhLoad] bhhJmp bhhCvA
             [bhhCvB, bhhLoad, bhhJmp] s (bhhEntrySEnd s) rfl rfl (by decide) hnt (bhhEntry_thread s) (by decide)
           rw [he] at hrun; exact absurd hrun (by simp)
    | 3 => obtain ⟨e, he⟩ := runBlock_oof bhhCtx bhhEntry 3 [bhhCvA, bhhCvB, bhhLoad] bhhJmp bhhCvA
             [bhhCvB, bhhLoad, bhhJmp] s (bhhEntrySEnd s) rfl rfl (by decide) hnt (bhhEntry_thread s) (by decide)
           rw [he] at hrun; exact absurd hrun (by simp)
    | (j+4) =>
      rw [show j+4 = 3+(j+1) from by omega, bhhEntry_runBlock s j hnh] at hrun
      injection hrun with h; subst h
      obtain ⟨ha0, hb0, hc0⟩ := bhhEntrySEnd_lookup s
      rw [hcv] at ha0 hb0
      refine ⟨by simpa [jumpTo, updateVar] using hnh, by simp [jumpTo, updateVar, hcv], fun _ => ⟨ha0, hb0, ?_⟩⟩
      rw [show lookupVar "c" (jumpTo "next" (bhhEntrySEnd s)) = lookupVar "c" (bhhEntrySEnd s) from rfl, hc0]
      rfl
  · -- next: RETURN never yields OK (ADD errors on undefined b/c, RETURN errors on undefined a, else Halt)
    rcases hlb : lookupVar "b" s with _ | wb
    · match f' with
      | 0 => rw [runBlock_no_phi 0 bhhCtx bhhNext s bhhMul [bhhRet] rfl (by decide)] at hrun
             simp [execBlock] at hrun
      | (j+1) =>
        rw [runBlock_body_head_error bhhCtx bhhNext bhhMul [bhhRet] s "undefined operand" j rfl
          (by decide) (bhhMul_error_b s hlb) (by decide) (by decide)] at hrun
        exact absurd hrun (by simp)
    · rcases hlc : lookupVar "c" s with _ | wc
      · match f' with
        | 0 => rw [runBlock_no_phi 0 bhhCtx bhhNext s bhhMul [bhhRet] rfl (by decide)] at hrun
        ...
        | (j+1) =>
          rw [runBlock_body_head_error bhhCtx bhhNext bhhMul [bhhRet] s "undefined operand" j rfl
            (by decide) (bhhMul_error_c s wb hlb hlc) (by decide) (by decide)] at hrun
          exact absurd hrun (by simp)
      · rcases hla : lookupVar "a" s with _ | wa
        · match f' with
          | 0 => rw [runBlock_no_phi 0 bhhCtx bhhNext s bhhMul [bhhRet] rfl (by decide)] at hrun
                 simp [execBlock] at hrun
          | 1 => obtain ⟨e, he⟩ := runBlock_oof bhhCtx bhhNext 1 [bhhMul] bhhRet bhhMul [bhhRet] s
                   (bhhNextSEnd s wb wc) rfl rfl (by decide)
                   (by intro i hi; simp only [List.mem_singleton] at hi; subst hi; decide)
                   (bhhNext_thread s wb wc hlb hlc) (by decide)
                 rw [he] at hrun; exact absurd hrun (by simp)
          | (j+2) =>
            rw [show j+2 = 1+(j+1) from by omega, bhhNext_error_a s j wb wc hlb hlc hla] at hrun
            exact absurd hrun (by simp)
        · match f' with
          | 0 => rw [runBlock_no_phi 0 bhhCtx bhhNext s bhhMul [bhhRet] rfl (by decide)] at hrun
                 simp [execBlock] at hrun
          | 1 => obtain ⟨e, he⟩ := runBlock_oof bhhCtx bhhNext 1 [bhhMul] bhhRet bhhMul [bhhRet] s
                 ...
                 rw [he] at hrun; exact absurd hrun (by simp)
          | (j+2) =>
            rw [show j+2 = 1+(j+1) from by omega, bhhNext_halts s j wb wc wa hlb hlc hla] at hrun
            exact absurd hrun (by simp)

/-- **Non-vacuity**: `runContext 10 bhhCtx vs` halts for every admitted state (callvalue zero).
    Chains `entry → next`, ending in `next`'s RETURN halt. -/
theorem bhhFn_halts (vs : VenomState) (hnh : vs.halted = false)
    (hcv : vs.callCtx.callvalue = EvmYul.UInt256.ofNat 0) :
    ∃ vs', runContext 10 bhhCtx vs = ExecResult.Halt vs' := by
  have h0 : runContext 10 bhhCtx vs
      = runBlocks 10 bhhCtx bhhFn { vs with prevBb := none, currentBb := "entry", instIdx := 0 } := by
    simp [runContext, runFunction, bhhCtx, bhhFn, lookupFunction, fnEntryLabel, bhhEntry]
  set s0 : VenomState := { vs with prevBb := none, currentBb := "entry", instIdx := 0 } with hs0
  have hnh0 : s0.halted = false := by simpa [hs0] using hnh
  have hcv0 : s0.callCtx.callvalue = EvmYul.UInt256.ofNat 0 := by simpa [hs0] using hcv
  set s1 : VenomState := jumpTo "next" (bhhEntrySEnd s0) with hs1
  have hentry : runBlock 9 bhhCtx bhhEntry s0 = ExecResult.OK s1 := by
    rw [show (9 : Nat) = 3 + (5 + 1) from by omega]; exact bhhEntry_runBlock s0 5 hnh0
  have hlk0 : lookupBlock s0.currentBb bhhFn.blocks = some bhhEntry := rfl
  have hnh1 : s1.halted = false := by rw [hs1]; simpa [jumpTo, updateVar] using hnh0
  have hstep0 : runBlocks 10 bhhCtx bhhFn s0 = runBlocks 9 bhhCtx bhhFn s1 :=
    runBlocks_step_of_block (fuel := 9) hlk0 hentry hnh1
  obtain ⟨ha1, hb1, hc1⟩ := bhhEntrySEnd_lookup s0
  rw [hcv0] at ha1 hb1
  have ha1' : lookupVar "a" s1 = some (EvmYul.UInt256.ofNat 0) := ha1
  have hb1' : lookupVar "b" s1 = some (EvmYul.UInt256.ofNat 0) := hb1
  have hc1' : lookupVar "c" s1 = some (bhhVal s0) := by
    rw [show lookupVar "c" s1 = lookupVar "c" (bhhEntrySEnd s0) from rfl, hc1]
  have hlk1 : lookupBlock s1.currentBb bhhFn.blocks = some bhhNext := rfl
  have hhalt := bhhNext_halts s1 6 (EvmYul.UInt256.ofNat 0) (bhhVal s0)
    (EvmYul.UInt256.ofNat 0) hb1' hc1' ha1'
  rw [show (1 : Nat) + (6 + 1) = 8 from by omega] at hhalt
  rw [h0, hstep0]
  exact ⟨_, runBlocks_haltDirect_of_block hlk1 hhalt⟩
...
set_option maxHeartbeats 4000000 in
/-- **A BLOBHASH capstone.** `codegen_correct` for
    `entry: %a=CALLVALUE ; %b=CALLVALUE ; %c=BLOBHASH %a ; JMP next` /
    `next: %e=MUL %b %c ; RETURN %e, %a`. `entry`'s BLOBHASH exercises the 1-input transient-read arm
    (`RegularStepG`'s BLOBHASH disjunct, `asmStep_tload_ok`), emitted `DUP2 ; BLOBHASH`, leaving `c = bhhVal s`
    (context-dependent). `next`'s MUL consumes the top two `[b, c]` (so `a` is never buried — no f1a
    reorder); `b = 0` ⇒ `e = 0` (`uint256_zero_mul`) empties the return window. `next` ends in a
    body-then-RETURN handled by `hsupplyW_regularReturnTo`; `entry` via `hsupplyW_regularJmp`. -/
theorem codegen_correct_bhhFn_recipeW {lo : AssocList String Nat} {vs : VenomState} {as : AsmState}
    (hvshalt : vs.halted = false) (hzero : vs.callCtx.callvalue = EvmYul.UInt256.ofNat 0)
    (hrel : venomAsmRel lo (initPlanState 0) vs as) (haspc : as.pc = 0) :
    (match runContext 10 bhhCtx vs with
     | ExecResult.Halt vs' => ∃ as', runAsm (asmResolve (executePlan (generateFnPlan bhhFn 0 0).get!.1)).1.length (asmResolve (executePlan (generateFnPlan bhhFn 0 0).get!.1)).2 (asmResolve (executePlan (generateFnPlan bhhFn 0 0).get!.1)).1 as = AsmResult.AsmHalt as' ∧ finalStateRel vs' as'
     | ExecResult.Abort AbortType.RevertAbort vs' => ∃ as', runAsm (asmResolve (executePlan (generateFnPlan bhhFn 0 0).get!.1)).1.length (asmResolve (executePlan (generateFnPlan bhhFn 0 0).get!.1)).2 (asmResolve (executePlan (generateFnPlan bhhFn 0 0).get!.1)).1 as = AsmResult.AsmRevert as' ∧ finalStateRel vs' as'
     | ExecResult.Abort AbortType.ExHaltAbort vs' => ∃ as', runAsm (asmResolve (executePlan (generateFnPlan bhhFn 0 0).get!.1)).1.length (asmResolve (executePlan (generateFnPlan bhhFn 0 0).get!.1)).2 (asmResolve (executePlan (generateFnPlan bhhFn 0 0).get!.1)).1 as = AsmResult.AsmFault as' ∧ finalStateRel vs' as'
     | _ => True) := by
  have hfnready : ∀ bb ∈ bhhFn.blocks, ∀ inst ∈ bb.instructions, codegenReadyInst inst := by
    intro bb hbb inst hinst
    simp only [bhhFn, List.mem_cons, List.not_mem_nil, or_false] at hbb
    rcases hbb with rfl | rfl
    · simp only [bhhEntry, List.mem_cons, List.not_mem_nil, or_false] at hinst
      rcases hinst with rfl | rfl | rfl | rfl <;> (unfold codegenReadyInst; decide)
    · simp only [bhhNext, List.mem_cons, List.not_mem_nil, or_false] at hinst
      rcases hinst with rfl | rfl <;> (unfold codegenReadyInst; decide)
  have hgen : generateFnPlan bhhFn 0 0
      = some ((generateFnPlan bhhFn 0 0).get!.1, (generateFnPlan bhhFn 0 0).get!.2) := rfl
  have hpsE : psOfFn (fnPlanFuel bhhFn) bhhFn 0 0 "entry" = initPlanState 0 :=
    psOfFn_entry rfl hfnready (by simp only [fnPlanFuel]; omega)
  refine codegen_correct_ofBlocks_recipeW_inv bhhInv
    (lo := lo) (pcOf := pcOfLabel (asmResolve (executePlan (generateFnPlan bhhFn 0 0).get!.1)).1)
    (psOf := psOfFn (fnPlanFuel bhhFn) bhhFn 0 0)
    (wOf := fun l => (asmResolve (executePlan (generateFnPlan bhhFn 0 0).get!.1)).1.length - pcOfLabel (asmResolve (executePlan (generateFnPlan bhhFn 0 0).get!.1)).1 l)
    (offsets := (computeLabelOffsets (executePlan (generateFnPlan bhhFn 0 0).get!.1)).2)
    (fuel := 10) (ctx := bhhCtx) (fn := bhhFn) (fnEom := 0) (lblCtr := 0)
    (entryName := "main") (entryLbl := "entry")
    (ops := (generateFnPlan bhhFn 0 0).get!.1) (psFinal := (generateFnPlan bhhFn 0 0).get!.2)
    hgen rfl rfl rfl ?_ ?_ bhhInv_pres ?_
    ⟨by simpa using hvshalt, by simpa using hzero, fun h => by simp at h⟩
  case _ =>
    intro bb hbb s
    simp only [bhhFn, List.mem_cons, List.not_mem_nil, or_false] at hbb
    rcases hbb with rfl | rfl
    · simp [runBlock, evalPhis, execBlock, bhhEntry, bhhCvA]
    · simp [runBlock, evalPhis, execBlock, bhhNext, bhhMul]
  case _ =>
    intro bb hbb s asm N k hE hinv hlbleq
    obtain ⟨⟨bb0, hlk_s, hvrel, hpc_asm⟩, hwN, hhalt⟩ := hE
    simp only [bhhFn, List.mem_cons, List.not_mem_nil, or_false] at hbb
    ...
    · -- entry: CALLVALUE ×2 + ADD (duplicating) + JMP next
      ...
      have hnt : ∀ inst ∈ [bhhCvA, bhhCvB, bhhLoad], isTerminator inst.opcode = false := by
        ...
        rcases hi with rfl | rfl | rfl <;> decide
      match k with
      | 0 => exact Or.inl (runBlock_oof bhhCtx bhhEntry 1 [bhhCvA, bhhCvB, bhhLoad] bhhJmp bhhCvA
               [bhhCvB, bhhLoad, bhhJmp] s (bhhEntrySEnd s) rfl rfl (by decide) hnt (bhhEntry_thread s) (by decide))
      | 1 => exact Or.inl (runBlock_oof bhhCtx bhhEntry 2 [bhhCvA, bhhCvB, bhhLoad] bhhJmp bhhCvA
               [bhhCvB, bhhLoad, bhhJmp] s (bhhEntrySEnd s) rfl rfl (by decide) hnt (bhhEntry_thread s) (by decide))
      | 2 => exact Or.inl (runBlock_oof bhhCtx bhhEntry 3 [bhhCvA, bhhCvB, bhhLoad] bhhJmp bhhCvA
               [bhhCvB, bhhLoad, bhhJmp] s (bhhEntrySEnd s) rfl rfl (by decide) hnt (bhhEntry_thread s) (by decide))
      | (j+3) =>
      rw [show j+3+1 = ([bhhCvA, bhhCvB, bhhLoad] : List Instruction).length + (j+1) from by
        simp only [List.length_cons, List.length_nil]; omega]
      refine Or.inr (hsupplyW_regularJmp
        (liveness := exLiveness) (dfg := DfgAnalysis.empty) (cfg := exCfg) (restFuel := j)
        (front := [bhhCvA, bhhCvB, bhhLoad]) (jmpInst := bhhJmp) (hd := bhhCvA) (tl := [bhhCvB, bhhLoad, bhhJmp])
        (nextLiveness := ["a", "b", "c"]) (curBbLabel := "entry") (dem := 1) (S := [])
        (ps0 := initPlanState 0) (lbl := "next") (off := 9) (bb' := bhhNext) (sEnd := bhhEntrySEnd s)
        (hbb := rfl) (hjmpop := rfl) (hoperands := rfl) (hcons := rfl) (hphi := by decide)
        (hnonterm := hnt) (hthread := bhhEntry_thread s) (hnothalt := by simpa [updateVar] using hhalt)
        (hreg := bhhEntry_body)
        (hsd := ⟨by intro op; rfl, by simp [initPlanState], by intro z hz; simp [initPlanState] at hz⟩)
        (hsv := by simp [StackIsVars, initPlanState])
        (hrel := by rw [hpsE] at hvrel; exact hvrel)
    ...
    · -- next: the CONSUMING ADD + body-then-RETURN
      ...
      have hpc7 : asm.pc = 7 := by rw [hpc_asm]; decide
      match k with
      | 0 => exact Or.inl (runBlock_oof bhhCtx bhhNext 1 [bhhMul] bhhRet bhhMul [bhhRet] s
               (bhhNextSEnd s (EvmYul.UInt256.ofNat 0) (bhhVal s)) rfl rfl (by decide)
               (by intro i hi; simp only [List.mem_singleton] at hi; subst hi; decide)
               (bhhNext_thread s _ _ hb0 hc0) (by decide))
      | (j+1) =>
      rw [show j+1+1 = ([bhhMul] : List Instruction).length + (j+1) from by
        simp only [List.length_cons, List.length_nil]; omega]
      refine Or.inr (hsupplyW_regularReturnTo
        (liveness := exLiveness) (dfg := DfgAnalysis.empty) (cfg := exCfg) (restFuel := j)
        (front := [bhhMul]) (retInst := bhhRet) (hd := bhhMul) (tl := [bhhRet])
        (nextLiveness := ["e", "a"]) (curBbLabel := "next") (dem := 1)
        (S := ["a", "b", "c"]) (Sn := ["a", "e"])
        (ps0 := psOfFn (fnPlanFuel bhhFn) bhhFn 0 0 "next") (offv := "e") (szv := "a") (base := [])
        (woff := EvmYul.UInt256.ofNat 0) (wsz := EvmYul.UInt256.ofNat 0)
        (sEnd := bhhNextSEnd s (EvmYul.UInt256.ofNat 0) (bhhVal s))
        (hbb := rfl) (hop := rfl) (hoperands := rfl) (hcons := rfl) (hphi := by decide)
        (hnonterm := by intro i hi; simp only [List.mem_singleton] at hi; subst hi; decide)
        (hthread := bhhNext_thread s _ _ hb0 hc0)
        (hevaloff := by rw [show evalOperand (Operand.Var "e")
              (bhhNextSEnd s (EvmYul.UInt256.ofNat 0) (bhhVal s))
            = lookupVar "e" (bhhNextSEnd s (EvmYul.UInt256.ofNat 0) (bhhVal s)) from rfl,
            (bhhNextSEnd_lookup s (EvmYul.UInt256.ofNat 0) (bhhVal s)).1, uint256_zero_mul])
        (hevalsz := by rw [show evalOperand (Operand.Var "a")
              (bhhNextSEnd s (EvmYul.UInt256.ofNat 0) (bhhVal s))
            = lookupVar "a" (bhhNextSEnd s (EvmYul.UInt256.ofNat 0) (bhhVal s)) from rfl,
            (bhhNextSEnd_lookup s (EvmYul.UInt256.ofNat 0) (bhhVal s)).2]; exact ha0)
        (hvoff := by rw [show operandVal (bhhNextSEnd s (EvmYul.UInt256.ofNat 0) (bhhVal s)) lo
              (Operand.Var "e")
            = lookupVar "e" (bhhNextSEnd s (EvmYul.UInt256.ofNat 0) (bhhVal s)) from rfl,
            (bhhNextSEnd_lookup s (EvmYul.UInt256.ofNat 0) (bhhVal s)).1, uint256_zero_mul])
        (hvsz := by rw [show operandVal (bhhNextSEnd s (EvmYul.UInt256.ofNat 0) (bhhVal s)) lo
              (Operand.Var "a")
            = lookupVar "a" (bhhNextSEnd s (EvmYul.UInt256.ofNat 0) (bhhVal s)) from rfl,
            (bhhNextSEnd_lookup s (EvmYul.UInt256.ofNat 0) (bhhVal s)).2]; exact ha0)
        (hready := bhhNext_ready)
        (hsd := ⟨by intro op; rfl, by decide, by
          intro z hz
          rw [show (psOfFn (fnPlanFuel bhhFn) bhhFn 0 0 "next").stack
            = [Operand.Var "a", Operand.Var "b", Operand.Var "c"] from rfl] at hz
          simp only [List.mem_cons, List.not_mem_nil, or_false] at hz
          rcases hz with h | h | h <;> injection h with h' <;> subst h'
          · exact ⟨_, ha0⟩
        ...
        (hlenu := by simp [EvmYul.uint256_ofNat_toNat]) (hw := by decide))
  case _ =>
    refine ⟨⟨bhhEntry, rfl, ?_, ?_⟩, ?_, hvshalt⟩
    · show venomAsmRel lo (psOfFn (fnPlanFuel bhhFn) bhhFn 0 0 "entry") _ as
      rw [hpsE]; exact hrel
    · show as.pc = pcOfLabel (asmResolve (executePlan (generateFnPlan bhhFn 0 0).get!.1)).1 "entry"
      rw [haspc]; exact (pcOfLabel_entry_zero rfl hfnready hgen).symm
    · show (asmResolve (executePlan (generateFnPlan bhhFn 0 0).get!.1)).1.length - pcOfLabel (asmResolve (executePlan (generateFnPlan bhhFn 0 0).get!.1)).1 "entry" ≤ (asmResolve (executePlan (generateFnPlan bhhFn 0 0).get!.1)).1.length
      omega
