...
A pure Lean Keccak-256 based on FIPS PUB 202 (SHA-3) and the original Keccak
specification; the function selector is the first 4 bytes of
Keccak-256(signature). Self-contained over `Array UInt64`/`ByteArray`, with
`selectorBytes` adapting the output to the `List UInt8` the codec uses.
Executable, not verified: nothing downstream proves anything *about* the hash —
the selector tests pin known vectors (`EvmAbi.Tests`).
-/



/-!
# Keccak-256 and the four-byte function selector (downstream of evm-abi-lean)

Lives here rather than in `evm-abi-lean`: that library's scope is the ABI
**codec roundtrip** over the type grammar, and a hash function is a separate
primitive. The selector level is a consumer concern -- it is this repo that needs
a *pure Lean* keccak so `AbiCrossval` can recompute the dispatcher's pinned
selector constants inside the kernel (`decide +kernel`), which EVMYulLean's own
`ffi.KEC` cannot do.
-/

namespace EvmYul.Venom.AbiLean.Hash
...
/-- The selector as this branch's `List UInt8`. -/
def selectorBytes (signature : String) : List UInt8 :=
  (functionSelector signature).data.toList



end EvmYul.Venom.AbiLean.Hash
