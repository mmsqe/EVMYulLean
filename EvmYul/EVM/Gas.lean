...
def Cselfdestruct (s : EVM.State) : ℕ :=
  let r := AccountAddress.ofUInt256 (s.stack[0]?.getD default)
...
def Csload (μₛ : Stack UInt256) (A : Substate) (I : ExecutionEnv .EVM) : ℕ :=
  if A.accessedStorageKeys.contains (I.codeOwner, (μₛ[0]?.getD default))
  then Gwarmaccess
  else Gcoldsload
...
def C' (s : State) (instr : Operation .EVM) : ℕ :=
  ...
  match instr with
    | .SSTORE => Csstore s
    | .TSTORE => Ctstore
    | .EXP => let μ₁ := (μₛ[1]?.getD default); if μ₁ == ⟨0⟩ then Gexp else Gexp + Gexpbyte * (1 + Nat.log 256 μ₁.toNat) -- TODO(check) I think this floors by itself. cf. H.1. YP.
    | .EXTCODECOPY => Caccess (AccountAddress.ofUInt256 (μₛ[0]?.getD default)) A + Gcopy * (((μₛ[3]?.getD default).toNat + 31) / 32)
    | .LOG0 => Glog + Glogdata * (μₛ[1]?.getD default).toNat
    | .LOG1 => Glog + Glogdata * (μₛ[1]?.getD default).toNat +     Glogtopic
    | .LOG2 => Glog + Glogdata * (μₛ[1]?.getD default).toNat + 2 * Glogtopic
    | .LOG3 => Glog + Glogdata * (μₛ[1]?.getD default).toNat + 3 * Glogtopic
    | .LOG4 => Glog + Glogdata * (μₛ[1]?.getD default).toNat + 4 * Glogtopic
    | .SELFDESTRUCT => Cselfdestruct s
    | .CREATE => Gcreate + R (μₛ[2]?.getD default).toNat
    | .CREATE2 => let μ₂ := (μₛ[2]?.getD default); Gcreate + Gkeccak256word * ((μ₂.toNat + 31) / 32) + R μ₂.toNat
    | .KECCAK256 => Gkeccak256 + Gkeccak256word * (((μₛ[1]?.getD default).toNat + 31) / 32)
    | .JUMPDEST => Gjumpdest
    | .SLOAD => Csload μₛ A I
    | .TLOAD => Ctload
    ...
    -/
    | .CALL =>         Ccall (AccountAddress.ofUInt256 (μₛ[1]?.getD default)) (AccountAddress.ofUInt256 (μₛ[1]?.getD default)) (μₛ[2]?.getD default) (μₛ[0]?.getD default) σ μ A
    | .CALLCODE =>     Ccall (AccountAddress.ofUInt256 (μₛ[1]?.getD default))          s.executionEnv.codeOwner (μₛ[2]?.getD default) (μₛ[0]?.getD default) σ μ A
    | .DELEGATECALL => Ccall (AccountAddress.ofUInt256 (μₛ[1]?.getD default))          s.executionEnv.codeOwner    ⟨0⟩ (μₛ[0]?.getD default) σ μ A
    | .STATICCALL =>   Ccall (AccountAddress.ofUInt256 (μₛ[1]?.getD default)) (AccountAddress.ofUInt256 (μₛ[1]?.getD default))    ⟨0⟩ (μₛ[0]?.getD default) σ μ A
    | .BLOBHASH => HASH_OPCODE_GAS
    | w =>
      if w ∈ Wcopy then Gverylow + Gcopy * (((μₛ[2]?.getD default).toNat + 31) / 32) else
      if w ∈ Wextaccount then Caccess (AccountAddress.ofUInt256 (μₛ[0]?.getD default)) A else
...
def memoryExpansionCost (s : EVM.State) (instr : Operation .EVM) : ℕ :=
 ...
 where
  μᵢ' : UInt256 :=
    match instr with
      | .KECCAK256 => .ofNat <| MachineState.M s.toMachineState.activeWords.toNat (s.stack[0]?.getD default).toNat (s.stack[1]?.getD default).toNat
      | .CALLDATACOPY | .CODECOPY => .ofNat <| MachineState.M s.toMachineState.activeWords.toNat (s.stack[0]?.getD default).toNat (s.stack[2]?.getD default).toNat
      | .MCOPY => .ofNat <| MachineState.M s.toMachineState.activeWords.toNat (max (s.stack[0]?.getD default).toNat (s.stack[1]?.getD default).toNat) (s.stack[2]?.getD default).toNat
      | .EXTCODECOPY => .ofNat <| MachineState.M s.toMachineState.activeWords.toNat (s.stack[1]?.getD default).toNat (s.stack[3]?.getD default).toNat
      | .RETURNDATACOPY => .ofNat <| MachineState.M s.toMachineState.activeWords.toNat (s.stack[0]?.getD default).toNat (s.stack[2]?.getD default).toNat
      | .MLOAD | .MSTORE => .ofNat <| MachineState.M s.toMachineState.activeWords.toNat (s.stack[0]?.getD default).toNat 32
      | .MSTORE8 => .ofNat <| MachineState.M s.toMachineState.activeWords.toNat (s.stack[0]?.getD default).toNat 1
      | .LOG0 | .LOG1 | .LOG2 | .LOG3 | .LOG4 =>
        .ofNat <| MachineState.M s.toMachineState.activeWords.toNat (s.stack[0]?.getD default).toNat (s.stack[1]?.getD default).toNat
      | .CREATE | .CREATE2 => .ofNat <| MachineState.M s.toMachineState.activeWords.toNat (s.stack[1]?.getD default).toNat (s.stack[2]?.getD default).toNat
      | .CALL | .CALLCODE =>
        let m : ℕ := MachineState.M s.toMachineState.activeWords.toNat (s.stack[3]?.getD default).toNat (s.stack[4]?.getD default).toNat
        .ofNat <| MachineState.M m (s.stack[5]?.getD default).toNat (s.stack[6]?.getD default).toNat
      | .DELEGATECALL | .STATICCALL =>
        let m : ℕ:= MachineState.M s.toMachineState.activeWords.toNat (s.stack[2]?.getD default).toNat (s.stack[3]?.getD default).toNat
        .ofNat <| MachineState.M m (s.stack[4]?.getD default).toNat (s.stack[5]?.getD default).toNat
      | .RETURN | .REVERT => .ofNat <| MachineState.M s.toMachineState.activeWords.toNat (s.stack[0]?.getD default).toNat (s.stack[1]?.getD default).toNat
