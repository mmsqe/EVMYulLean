...
Note: users are recommended to use `Std.TreeMap` instead of `Batteries`.
...
which have more complex proofs that are delayed to `Batteries.Lemmas`.
-/
...
instance : SDiff (RBSet α cmp) := ⟨RBSet.sdiff⟩
